...
def test_record_failure_preserves_extension_and_metadata_when_not_resupplied(tmp_path):
    store = SqliteStateStore(str(tmp_path / "state.db"))
    store.record_failure("src1", "doc1", "first", 1, extension="pdf", metadata='{"origin": {"kind": "file"}}')
    # Deuxième échec sans extension/metadata explicites : les valeurs précédentes doivent être conservées.
    store.record_failure("src1", "doc1", "second", 2, extension=None, metadata=None)
    failed = store.get_failed("src1", max_retries=5)
    assert failed == [("doc1", 2, "second", "pdf", '{"origin": {"kind": "file"}}')]
    store.close()


def test_record_failure_updates_extension_and_metadata_when_resupplied(tmp_path):
    store = SqliteStateStore(str(tmp_path / "state.db"))
    store.record_failure("src1", "doc1", "first", 1, extension="pdf", metadata='{"origin": {"kind": "file"}}')
    store.record_failure("src1", "doc1", "second", 2, extension="docx", metadata='{"origin": {"kind": "https"}}')
    failed = store.get_failed("src1", max_retries=5)
    assert failed == [("doc1", 2, "second", "docx", '{"origin": {"kind": "https"}}')]
    store.close()
...
def test_schema_migration_adds_extension_and_metadata_columns_from_original_schema(tmp_path):
    """Une base créée avant l'ajout des colonnes `extension`/`metadata` doit
    être migrée automatiquement à l'ouverture (ALTER TABLE ADD COLUMN)."""
    ...
    failed = store.get_failed("src1", max_retries=5)
    assert failed == [("doc1", 1, "old-error", None, None)]
    store.close()


def test_schema_migration_adds_extension_and_metadata_columns(tmp_path):
    """Une base créée avant l'ajout des colonnes `extension`/`metadata` (mais
    ayant déjà `origin`) doit être migrée automatiquement à l'ouverture."""
    db_path = tmp_path / "legacy.db"
    conn = sqlite3.connect(str(db_path))
    conn.execute(
        """
    ...
    conn.execute(
        "INSERT INTO failed_documents (source_name, doc_id, error_count, last_error, origin) VALUES (?, ?, ?, ?, ?)",
        ("src1", "doc1", 1, "old-error", '{"kind": "file"}'),
    )
    conn.commit()
    conn.close()
